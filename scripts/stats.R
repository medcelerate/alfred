...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "BaseContent.html")
		}
	}
}
print(warnings())
...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "BaseQualities.html")
		}
	}
}
print(warnings())
...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "ReadLength.html")
		}
	}
}
print(warnings())
...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "MappingQuality.html")
		}
	}
}
print(warnings())
	...
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (tc > 0) {
			...
			if (device == "pdf") {
				...
				p1 = ggplotly(p1)
				htmlwidgets::saveWidget(as.widget(p1), "Coverage.html")
			}
		}
	}
}
	...
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (tc > 0) {
			...
			if (device == "pdf") {
				...
				p1 = ggplotly(p1)
				htmlwidgets::saveWidget(as.widget(p1), "InsertSize.html")
			}
		}
	}
}
...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "HomopolymerInDels.html")
		}
	}
}
print(warnings())
...
for (sid in unique(all$Sample)) {
	for (rg in unique(all[all$Sample == sid, ] $Library)) {
		...
		if (device == "pdf") {
			...
			p1 = ggplotly(p1)
			htmlwidgets::saveWidget(as.widget(p1), "InDelsSize.html")
		}
	}
}
print(warnings())
...
if (1) {
	...
	if (device == "pdf") {
		...
		p1 = ggplotly(p1)
		htmlwidgets::saveWidget(as.widget(p1), "GCContent.html")
	}
}
print(warnings())
	...
	for (sid in unique(all$Sample)) {
		for (rg in unique(all[all$Sample == sid, ] $Library)) {
			...
			if (device == "pdf") {
				...
				p1 = ggplotly(p1)
				htmlwidgets::saveWidget(as.widget(p1), "OnTargetRate.html")
			}
		}
	}
	print(warnings())
	...
	for (sid in unique(all$Sample)) {
		for (rg in unique(all[all$Sample == sid, ] $Library)) {
			...
			for (i in 1: nrow(sl)) {
				...
				for (j in 0: max(x$AvgCov)) {
					s = rbind(s, cbind(Sample = as.character(sl[i, "Sample"]), Library = as.character(sl[i, "Library"]), frac = mean(x$AvgCov >= j), cov = j));
				}
			}
			...
			if (device == "pdf") {
				...
				p1 = ggplotly(p1)
				htmlwidgets::saveWidget(as.widget(p1), "TargetCoverage.html")
			}
		}
	}
	print(warnings())
	...
	for (sid in unique(all$Sample)) {
		for (rg in unique(all[all$Sample == sid, ] $Library)) {
			...
			if (device == "pdf") {
				...
				p1 = ggplotly(p1)
				htmlwidgets::saveWidget(as.widget(p1), "PhasedBlockLength.html")
			}
		}
	}
	print(warnings())
...
for (i in 1: nrow(all)) {
	for (j in 1: ncol(all)) {
		p1 = p1 + annotate("text", x = j, y = nrow(all) - i, label = all[i, j], size = 2);
	}
}
if (device == "pdf") {
	print(p1)
	...
	p1 = ggplotly(p1)
	htmlwidgets::saveWidget(as.widget(p1), "Metrics.html")
}
print(warnings())
